"""arXiv + Semantic Scholar integration for citation graph."""
...
_semantic_scholar_last_call: list[float] = [0.0]  # Mutable so we avoid global
_semantic_scholar_lock = asyncio.Lock()


async def fetch_arxiv_metadata(arxiv_id: str) -> dict[str, Any] | None:
    """Fetch paper metadata from arXiv API."""
    # Normalize ID (handle URLs, versioned IDs)
    ...
    return {
        ...
        "authors": authors,
        "categories": categories[:5],  # Limit topics
...
async def fetch_citations_semantic_scholar(arxiv_id: str) -> dict[str, Any]:
    ...
    # Extract arXiv IDs from references/citations
    references = []
    ...
    return {"references": references[:20], "citations": citations[:20]}  # Limit for sanity
